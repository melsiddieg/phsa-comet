...
if( count($_POST) )
{
	...
	if( isset($_POST["submit_update"]) && isset($_POST["map_id"]) && is_numeric($_POST["map_id"]) && isset($_POST["update_map_code"]) && isset($_POST["update_map_vocabulary"]) && isset($_SESSION["PHSA_PRIV_MAP"]) && $_SESSION["PHSA_PRIV_MAP"] === "1" )
	{
		##UPDATE
	...
	elseif( isset($_POST["submit_add"]) && isset($_POST["new_map_code"]) && isset($_POST["new_map_vocabulary"]) && isset($_SESSION["PHSA_PRIV_MAP"]) && $_SESSION["PHSA_PRIV_MAP"] === "1" )
	{
		##ADD
	...
	elseif( isset($_POST["exclude_status"]) && isset($_POST["comment_text"]) && isset($_POST["exclude_status_submit"]) && isset($_SESSION["PHSA_PRIV_MAP"]) && $_SESSION["PHSA_PRIV_MAP"] === "1" )
	{
		...
		if( is_numeric($ft_data["total_count"]) )
		{
			...
			{
				$sql = "update phsa_mr_sheets set total_mappable_count = total_mappable_count - " . $ft_data["total_count"] . " where id = " . $ft_data["sheet_id"] . " limit 1";
				$rs = $pdo->query($sql);
			}
	...
	elseif( isset($_POST["submit_approve"]) )
	{
		# APPROVE
		$sql = "
			insert into phsa_maps_snapshot (data_id, concept_id, date_time, approved_by)
			select $data_id, target_concept_id, now(), '" . $_SESSION["PHSA_UNAME"] . "'
			from phsa_all_maps where src_data_id = $data_id";
		$rs_a = $pdo->query($sql);
		
		$sql = "update phsa_all_maps_hx set approved_by = '" . $_SESSION["PHSA_UNAME"] . "', approved_dttm = now() where src_data_id = $data_id";
		$rs_a = $pdo->query($sql);
	}
	else
	{
...
<tr style="background-color:#f8fdff;"><td>
	...
	for( $r = 0, $One2M_index = 1 ; $r < $num_rows ; $r++, $One2M_index++ )
	{
		...
		$tr_bgcolor = get_tr_bgcolor($targets_arr[$r]["t_concept_code"], $targets_arr[$r]["m_concept_code"]);
	...
	//	print_r($targets_arr);
		...
		if( is_numeric($map_id) )
			echo "<input type='text' name='update_map_code' id='update_map_code' size='25' maxlength='25' value='" . $targets_arr[$r]["m_concept_code"] . "' />";
		echo "</td></tr>";
		...
		if( is_numeric($map_id) )
		{
			echo "<select name='update_map_vocabulary' id='update_map_vocabulary'>";
		...
		if( is_numeric($map_id) )
			echo "<tr><td colspan='2' align='center'><button name='submit_update' id='submit_update_btn' onclick=\"submit_update_rev($data_id, $map_id);\"> Update </button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button name='submit_delete' id='submit_delete_btn' onclick=\"if(confirm('Are you sure you want to delete this map?')) submit_delete_rev($data_id, $map_id); else return false;\"/> Delete </buttin></td></tr>";
	}
	...
	echo "</select></td></tr>";
	echo "<tr><td colspan='2' align='center'><button name='submit_add' id='submit_add_btn' onclick=\"submit_add_rev($data_id);\"> Add </button></td></tr>";
	echo "</table>";
...
echo "<div align='right'><button name='submit_add' id='submit_add_btn' onclick=\"submit_excl_update_rev($data_id);\">  Update </button></div></td></tr>";
echo "<tr><td colspan='3' align='center'>";

## Determining if there is any change(s) to be approved
$sql = "select 1 as dummy from phsa_all_maps_hx where src_data_id = $data_id and approved_by is null";
$rs = $pdo->query($sql);
if( ($ft = $rs->fetch(PDO::FETCH_ASSOC)) )
	echo "<button id='approve_btn' style='font-size:28px; background-color:#20b030; color:#f8f8ff; padding:10px 50px;' onclick=\"submit_approve_rev($data_id);\">Approve</button>";
else
	echo "<span style='font-size:18px; color:#008000;'>No changes to be approved!</span>";
echo "</td></tr>";
#######################
...
echo "<p></p><p><br/></p><h2>Change History</h2>";
...
?>
	...
	{
		?>
		if( document.getElementById("approve_btn") )
...
if( count($_POST) && isset($_POST["submit_approve"]) )
{
?>
	<script>
	var approved_tr = document.getElementById(<?php echo "\"tr_$data_id\""; ?>);
	approved_tr.bgColor = "#66d066";
